def min_loss_data_collator(features):
        ...
        for feature in features:
            ...
            input_ids = torch.tensor(feature["input_ids"]).clone().detach().to(device)  # Add batch dimension
            labels = input_ids.clone()  # Clone to avoid modifying the original

            with torch.inference_mode():
                loss_fct = torch.nn.CrossEntropyLoss(reduction='none')
                logits = model(input_ids.unsqueeze(0), labels=input_ids.unsqueeze(0)).logits
